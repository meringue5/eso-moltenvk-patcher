# Primary sources

- [MoltenVK Runtime User Guide](https://github.com/KhronosGroup/MoltenVK/blob/main/Docs/MoltenVK_Runtime_UserGuide.md)
- [MoltenVK Configuration Parameters](https://github.com/KhronosGroup/MoltenVK/blob/main/Docs/MoltenVK_Configuration_Parameters.md)
- [MoltenVK What's New](https://github.com/KhronosGroup/MoltenVK/blob/main/Docs/Whats_New.md)
- [MoltenVK v1.4.1 release](https://github.com/KhronosGroup/MoltenVK/releases/tag/v1.4.1)
- [Vulkan extension registry](https://registry.khronos.org/vulkan/)

Local claims about ESO's binary layout come from `file`, `otool`, `nm`,
`dyld_info`, byte-signature matching, and disassembly of a legally installed
local Steam copy. Proprietary inputs and extracted object files are not included
in this repository.

